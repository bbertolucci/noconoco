...
sampleplayer.CastPlayer = function(element) {
 ...
 var self = this;
        ...
        this.messageBus =
          this.receiverManager_.getCastMessageBus('urn:x-cast:com.bbproject.noconoco');
          ...
          // display the message from the sender
			...
			if(obj.message==1) {
				//{"message":1,"content":{"ratingFr":"","duration":3683,"resume":"","screenshot":"http:\/\/media.noco.tv\/screenshot\/nol\/512x288\/a\/e\/OT_S15s03_43180.jpg","showTT":"","familyTT":"OTO","theme":"Musique","idPartner":"1","seasonepisode":"Episode : 288","type":"Rubrique","banner":"http:\/\/media.noco.tv\/family\/slider\/nol\/ot.jpg"}} 
				
				$(".preview").css('background-image', 'url(' + content.screenshot + ')');
				var urlPartner = "";
				switch(content.idPartner) {
				case '12' : urlPartner = "http://media.noco.tv/partner/logo_player/LBL42.svg";
						  break;
				case '26' : urlPartner = "http://media.noco.tv/partner/icon/512x288/noco.jpg";
						  break;
				case '1' : urlPartner = "http://media.noco.tv/partner/logo_player/nol.svg";
						  break;
				case '4' : urlPartner = "http://media.noco.tv/partner/logo_player/oly.jpg";
						  break;
				case '9' : urlPartner = "http://media.noco.tv/partner/logo_player/tok.svg";
						  break;
				case '15' : urlPartner = "http://media.noco.tv/partner/logo_player/wak.svg";
						  break;
				case '24' : urlPartner = "http://media.noco.tv/partner/logo_player/wep.svg";
						  break;
				}
				$(".previewlogo").css('background-image', 'url(' + urlPartner + ')');
				$(".previewfamily").html(content.familyTT);
				$(".previewtitle").html(content.showTT);
				if (''==content.showTT)
					$(".previewfamily").css({ top: '250px' });
				else 
					$(".previewfamily").css({ top: '200px' });
				$(".previewepisode").html(content.seasonepisode);
				$(".previewresume").html(content.resume);
				$(".previewresume").css('background-image', 'url(' + content.banner + ')');
				$(".previewtype").html("<span>"+content.type+"</span><span>"+content.theme+"</span>");
				var totalSec = content.duration;
				var hours = parseInt( totalSec / 3600 ) % 24;
				var minutes = parseInt( totalSec / 60 ) % 60;
				var seconds = parseInt(totalSec % 60, 10);

				var result = (hours > 0 ? hours + ":" : "") + (minutes < 10 ? "0" + minutes : minutes) + ":" + (seconds  < 10 ? "0" + seconds : seconds);
				$("#logo").hide();
				$(".previewduration").html(result);
				$(".previewresume").show();
			} else if(obj.message==2) {
				//{"message":2,"content":{"selectLang":"fr","selectSub":"none","selectQua":"LQ","lang":["fr"],"sub":["none"],"qua":["HD_720","LQ","HQ","TV"]}}
				
				/*console.log(content.selectLang);
				console.log(content.selectSub);
				console.log(content.selectQua);
				console.log(content.lang);
				console.log(content.sub);
				console.log(content.qua);
				
				[75:com.bbproject.noconoco-26]: {"message":1,"content":{"ratingFr":"10","duration":71982,"resume":"","screenshot":"http:\/\/media.noco.tv\/screenshot\/wep\/512x288\/4\/0\/FILM_Maitre_des_Sorciers.jpg","showTT":"","familyTT":"Le Maître des Sorciers","theme":"Cinéma","idPartner":"24","seasonepisode":"","type":"Film","banner":"http:\/\/media.noco.tv\/family\/slider\/wep\/film.jpg"}}
				
				Message [75:com.bbproject.noconoco-26]: {"message":2,"content":{"selectLang":"fr","selectSub":"none","selectQua":"LQ","lang":["de","fr"],"sub":["none"],"qua":["HD_720","LQ","HQ","TV"]}} player.js:201
Message [75:com.bbproject.noconoco-26]: {"message":2,"content":{"selectLang":"de","selectSub":"fr","selectQua":"LQ","lang":["de","fr"],"sub":["fr"],"qua":["HD_720","HD_1080","HQ","LQ","TV"]}} player.js:201
Message [75:com.bbproject.noconoco-26]: {"message":2,"content":{"selectLang":"fr","selectSub":"none","selectQua":"LQ","lang":["de","fr"],"sub":["none"],"qua":["HD_720","LQ","HQ","TV"]}} player.js:201
Message [75:com.bbproject.noconoco-26]: {"message":2,"content":{"selectLang":"de","selectSub":"fr","selectQua":"LQ","lang":["de","fr"],"sub":["fr"],"qua":["HD_720","HD_1080","HQ","LQ","TV"]}} 
*/
				var htmllang="";
				for (var i = 0; i < content.lang.length; i++) {
					var cont = content.lang[i];
					htmllang+="<span "+((content.lang[i]==content.selectLang)?"state=\"active\"":"")+">"+cont+"</span>";
				}
				$(".previewlang").html(htmllang);
				
				if("none"==content.sub) {
					$(".previewsublang").html("");
				} else {
					htmllang="<span state=\"text\">Sous-titres:</span>";
					for (var i = 0; i < content.sub.length; i++) {
						var cont = content.sub[i];
						htmllang+="<span "+((content.sub[i]==content.selectSub)?"state=\"active\"":"")+">"+cont+"</span>";
					}
					$(".previewsublang").html(htmllang);
				}
				
				htmllang="";
				for (var i = 0; i < content.qua.length; i++) {
					var cont = content.qua[i];
					//if(content.qua[i]=="HD_720") cont = "720p";
					//else if(content.qua[i]=="HD_1080") cont = "1080p";
					htmllang+="<span "+((content.qua[i]==content.selectQua)?"state=\"active\"":"")+">"+cont+"</span>";
				}
				$(".previewquality").html(htmllang);
			}
          //displayText(event.data);
          // inform all senders on the CastMessageBus of the incoming message event
          // sender message listener will be invoked
          self.messageBus.send(event.senderId, event.data);
        }  
  /**
...
sampleplayer.CastPlayer.prototype.onEnded_ = function() {
  this.log_('onEnded');
  this.setState_(sampleplayer.State.STALLED, true);
};
